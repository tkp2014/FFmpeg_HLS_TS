...
//By tony
static char *subString(char *destin, const char *source, int start, int maxlen);
static time_t get_systime(void);
static int hls_idx_start(AVFormatContext *s);// Do idx file open.
...
static int hls_delete_old_segments(HLSContext *hls) {
	...
	/* By tony */
	execute_order++;
    av_log(hls, AV_LOG_DEBUG, "Function: %s||Line: %d||Order: %d\n", __FUNCTION__, __LINE__, execute_order);
...
static int hls_encryption_start(AVFormatContext *s)
{
	/* By tony */
	execute_order++;
    av_log(s, AV_LOG_DEBUG, "Function: %s||Line: %d||Order: %d\n", __FUNCTION__, __LINE__, execute_order);
                              ...
                              int64_t pos, int64_t size)
{
	/* By tony */
	execute_order++;
    av_log(NULL, AV_LOG_DEBUG, "Function: %s||Line: %d||Order: %d\n", __FUNCTION__, __LINE__, execute_order);

    hls_saveIdx(s);// get idx file.By tony
...
static void hls_free_segments(HLSSegment *p)
{
	/* By tony */
	execute_order++;
    av_log(p, AV_LOG_DEBUG, "Function: %s||Line: %d||Order: %d\n", __FUNCTION__, __LINE__, execute_order);
...
static void set_http_options(AVDictionary **options, HLSContext *c)
{
	/* By tony */
	execute_order++;
    av_log(c, AV_LOG_DEBUG, "Function: %s||Line: %d||Order: %d\n", __FUNCTION__, __LINE__, execute_order);
...
static int hls_start(AVFormatContext *s)
{
    ...
    //By tony
//    av_log(NULL, AV_LOG_DEBUG, "007c->wrap: %d, c->sequence: %d\n", c->wrap, c->sequence);
//    c->wrap = 18;


//    av_log(NULL, AV_LOG_DEBUG, "20160812c->basename: %s, c->wrap: %d, c->sequence: %ld\n", c->basename,c->wrap,c->sequence);
//    av_get_frame_filename(oc->filename, sizeof(oc->filename),
//                                      c->basename, c->wrap ? c->sequence % c->wrap : c->sequence);
    ...
    if (c->key_info_file) { //key information file.By tony
        ...
        if (err < 0)
       	...
       	hls_idx_start(s);//open idx file.By tony
...
static int hls_start_longTS(AVFormatContext *s)
{
		/* By tony */
		execute_order++;
		av_log(NULL, AV_LOG_DEBUG, "Function: %s\n", __FUNCTION__);
	...
	//	    else
	//	        if ((err = s->io_open(s, &oc->pb, oc->filename, AVIO_FLAG_WRITE, &options)) < 0) { //first time open the ts file.By tony
	//	            goto fail;
	//	        }
...
/**
 * Get sub string.By tony
 */
char *subString(char *destin, const char *source, int start, int maxlen)
{
	 char *p;
	 char *address;
	 int  n = 0; /*字符串的长度*/
...
/**
 * Get idx file. By tony.
 */
static int hls_get_idx(AVFormatContext *s)
{
	HLSContext *hls = s->priv_data;
    AVIOContext *idx_out = NULL;
    AVDictionary **options = NULL;
    char idx_filename[512];
    char *base_filename = av_basename(hls->avf->filename);
	char *tmp_ptr;
	int ret;
...
static int hls_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (is_ref_pkt)
        hls->duration = (double)(pkt->pts - hls->end_pts)  // duration(0/1). By tony
...
//	av_log(NULL, AV_LOG_INFO, "20160812av_compare_ts: %d\n", result); //By tony
    ...
    if (can_split && av_compare_ts(pkt->pts - hls->start_pts, st->time_base,
    	...
    	/* the value of av_compare_ts() is awalys 0 .By tony */
