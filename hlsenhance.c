 ...
 *      Author: ubuntu01
 *
 *  use start_number for the name of ts segments.
 */
...
/* The variables defined below all by tony */
const int g_merge_ts_num = 18; // number of ts per merged ts.
#define HLSDEBUG 1
/* End define */
...
typedef struct HLSContext {
    ...
    /* define AVIOContext for idx and index.By tony */
    AVIOContext *idx_out; // output idx
    AVIOContext *index_out; // output index
    time_t time_stamp;	// current system time
...
/* Function defined by tony */

/**
 * Do idx open
 */
static int hls_idx_init(AVFormatContext *s, AVDictionary *opt)
{
	/* Get the timestamp for idx and merged ts */
    struct tm *tmp;
	time_t now0;
    int err = 0;
    ...
    time(&now0);
    tmp = localtime(&now0);
    hls->time_stamp = mktime(tmp);

#if HLSDEBUG
 	av_log(NULL, AV_LOG_DEBUG, "Function: %s, Line: %d, hls->time_stamp: %d\n", __FUNCTION__, __LINE__, hls->time_stamp);
#endif


 	/* get dir name */
	char *dir;
	char *fn_copy = av_strdup(hls->avf->filename);

#if HLSDEBUG
 	av_log(NULL, AV_LOG_DEBUG, "Function: %s, Line: %d, fn_copy: %s\n", __FUNCTION__, __LINE__, fn_copy);
#endif
...
#if HLSDEBUG
 	av_log(NULL, AV_LOG_DEBUG, "Function: %s, Line: %d, dir: %s\n", __FUNCTION__, __LINE__, dir);
#endif

	/* write out normal index. */
 	char merge_ts_dir_name[256];
 	char merge_ts_name[256];
 	char merge_idx_name[256];
 	snprintf(merge_idx_name, sizeof(merge_idx_name), "%d.idx", hls->time_stamp);
 	snprintf(merge_ts_name, sizeof(merge_ts_name), "%d.ts", hls->time_stamp);

#if HLSDEBUG
 	av_log(NULL, AV_LOG_DEBUG, "Function: %s, Line: %d, merge_idx_name: %s\n", __FUNCTION__, __LINE__, merge_idx_name);
 	av_log(NULL, AV_LOG_DEBUG, "Function: %s, Line: %d, merge_ts_name: %s\n", __FUNCTION__, __LINE__, merge_ts_name);
#endif

	avio_printf(hls->index_out, "%s\n", merge_idx_name);
	avio_flush(hls->index_out);

//	av_log(NULL, AV_LOG_DEBUG, "20160905%s: \n", hls_filename);

	strcat(dir, "/");
	strcpy(merge_ts_dir_name, dir);
	strcat(merge_ts_dir_name, merge_ts_name);

#if HLSDEBUG
	av_log(NULL, AV_LOG_DEBUG, "dir+/: %s\n", dir);
#endif

	strcat(dir, merge_idx_name);

#if HLSDEBUG
	av_log(NULL, AV_LOG_DEBUG, "dir+merge_idx_name: %s\n", dir);
#endif

	/* must get the filename of idx which includes path.By tony */
	if ((err = s->io_open(s, &hls->idx_out, dir, AVIO_FLAG_WRITE, NULL)) < 0) {
		ff_format_io_close(s, &hls->idx_out);
		return err;
	}
	/* open ts */
	av_log(NULL, AV_LOG_DEBUG, "merge_ts_dir_name: %s\n", merge_ts_dir_name);
	if ((err = s->io_open(s, &hls->avf->pb, merge_ts_dir_name, AVIO_FLAG_WRITE, &opt)) < 0) {
	    av_dict_free(&opt);
	    return err;
	}
	av_free(fn_copy);
...
/**
 * Just write idx.By tony.
 */
static int hls_save_merge_idx(AVFormatContext *s)
{
    ...
    if (0 == hls->start_pos) {
    	prev_time = hls->time_stamp;
    	curr_time = hls->time_stamp;
    } else {
    	curr_time = prev_time + hls->duration;
    	prev_time = curr_time;
    }

	avio_printf(hls->idx_out, "%d \t%s \t%10"PRIi64" \t%10"PRIi64" \t%d\n",
			curr_time, av_basename(hls->avf->filename), hls->start_pos, hls->size, (long)hls->duration);
	avio_flush(hls->idx_out);

#if HLSDEBUG
    av_log(NULL,AV_LOG_DEBUG, "20160826timestramp:%d, filename:%s\t,size:%"PRIi64"\t,start_pos:%"PRIi64"\t,duration:%f\n",
    		curr_time, av_basename(hls->avf->filename), hls->size, hls->start_pos,hls->duration);
#endif
...
{
	hls_save_merge_idx(s); // save every ts info per idx.
...
/**
 * In order to get 3 minute ts
 */
static int hls_start_longts(AVFormatContext *s)
{
...
static int hls_start(AVFormatContext *s)
{
    ...
    if (c->key_info_file) {
        ...
        if (err < 0)
       	...
       	hls_idx_init(s, options);// open idx file.
    }
...
static int hls_write_header(AVFormatContext *s)
{
    ...
    char normal_index[256];
    const char *ptr_index = hls->basename;

#if HLSDEBUG
    av_log(NULL, AV_LOG_DEBUG, "Function: %s, Line: %d, ptr_index: %s\n", __FUNCTION__, __LINE__, ptr_index);
#endif

    int len_index = av_basename(ptr_index) - ptr_index;
    strncpy(normal_index, ptr_index, len_index);
    normal_index[len_index] = '\0';

#if HLSDEBUG
    av_log(NULL, AV_LOG_DEBUG, "Function: %s, Line: %d, normal_index: %s\n", __FUNCTION__, __LINE__, normal_index);
    av_log(NULL, AV_LOG_DEBUG, "Function: %s, Line: %d, len_index: %d\n", __FUNCTION__, __LINE__, len_index);
#endif

//    snprintf(normal_index, sizeof(normal_index), "%snormal.index", tmp); //path of normal.index.
    strcat(normal_index, "normal.index");

#if HLSDEBUG
    av_log(NULL, AV_LOG_DEBUG, "Function: %s, Line: %d, normal.index: %s\n", __FUNCTION__, __LINE__, normal_index);
#endif

    /*open normal index file.By tony*/
    if ((ret = s->io_open(s, &hls->index_out, normal_index, AVIO_FLAG_WRITE, NULL)) < 0) {
    	ff_format_io_close(s, &hls->index_out);
    	return ret;
    }
...
static int hls_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (can_split && av_compare_ts(pkt->pts - hls->start_pts, st->time_base,
        ...
        if (hls->flags & HLS_SINGLE_FILE) {
        	...
        	if (0 == (hls->number % g_merge_ts_num)) {
				ff_format_io_close(s, &hls->idx_out);//close idx g_idx_out for idx.By tony
				ff_format_io_close(s, &oc->pb);
				avio_flush(hls->index_out); // flush normal.index
				if (hls->vtt_avf) {
					ff_format_io_close(s, &hls->vtt_avf->pb);
				}
				ret = hls_start(s); //open pb and idx
        	} else {
        		hls_start_longts(s);
        	}
        }
...
static int hls_write_trailer(struct AVFormatContext *s)
{
    ...
    if (oc->pb) {
        ...
        ff_format_io_close(s, &hls->idx_out); //close idx file and flush
        ff_format_io_close(s, &hls->index_out); //close normal.index and flush
    }
...
AVOutputFormat ff_hlsenhance_muxer = {
    .name           = "hlsenhance",
